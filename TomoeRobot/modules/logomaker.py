...
from TomoeRobot.events import register
from TomoeRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./AnyaRobot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Anya Robot](https://t.me/Anyamanagerobot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./AnyaRobot/resources/fonts"):
            os.remove(font_)
...
async def logo_(event):
    ...
    if event.reply_to_msg_id:
        ...
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
        fpath_ = glob.glob("./Anyamanagerobot/resources/fonts/*")
        font_ = random.choice(fpath_)
    if not bg_:
        pics = []
        async for i in ubot2.iter_messages(
    ...
    if os.path.exists(bg_):
        os.remove(bg_)
    if os.path.exists(font_):
        if not font_.startswith("./Anyamanagerbot/resources/fonts"):
            os.remove(font_)
